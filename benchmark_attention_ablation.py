...
try:
    ...
    from training.quantum_transformer import QuantumFrontEndQCNN, QuantumAttention64
    
except ImportError as e:
    print(f"Error importing modules: {e}")
    sys.exit(1)

# --- Wrapper for QSANN to handle UNetBlock kwargs ---
class QSANNAdapter(QuantumAttention64):
    def __init__(self, in_channels, num_heads, **kwargs):
        # QuantumAttention64 expects: N_QUBITS, Q_DEPTH, qk_dim, n_heads
        # We map num_heads -> n_heads
        # We ignore in_channels (assuming C=64 matches N_QUBITS=6)
        
        valid_keys = ['N_QUBITS', 'Q_DEPTH', 'qk_dim', 'n_heads', 'tau', 'tau_trainable', 
                      'attn_gate_init', 'attn_dropout', 'qk_norm', 'force_fp32_attention', 'device_name']
        
        filtered_kwargs = {k: v for k, v in kwargs.items() if k in valid_keys}
        filtered_kwargs['n_heads'] = num_heads
        
        # Ensure N_QUBITS is 6 (default) for 64 dims
        # If in_channels != 64, this might be an issue, but we set C_model=64 in this benchmark.
        if in_channels != 64:
            print(f"WARNING: QSANNAdapter received in_channels={in_channels}, but QSANN is optimized for 64 dims (6 qubits).")
...
def run_experiment():
    ...
    print(f"Running Quantum Attention Ablation Experiment on {device}")
    ...
    # All use "use_qcnn_frontend": True (SOTA Convolution)
    # Varying Attention
    models_config = {
        "Baseline: Classic-Attn": {
            "type": "UNetBlock",
            "kwargs": {
        ...
        "QSANN: QSANN-Attn": {
            ...
            "kwargs": {
                ...
                "attention": True,
                "use_mlp_output": True,
                "quantum_adapter": QSANNAdapter(in_channels=C_model, num_heads=4, N_QUBITS=6, Q_DEPTH=4)
            }
        },
        "SOTA: SOTA-Q-Attn": {
            "type": "UNetBlock",
            "kwargs": {
                ...
                "attention": True,
                "use_mlp_output": True,
                "quantum_adapter": "training.quantum_transformer:QuantumAttentionHybridLite",
                "quantum_adapter_kwargs": {
                     "input_dim": C_model # Pass 64 explicitly
                }
            }
        }
    }
    ...
    print("\n" + "="*95)
    print("QUANTUM ATTENTION ABLATION REPORT")
    print("="*95)
    ...
    print(f"CONCLUSION: Optimal attention strategy is {best_model}.")
